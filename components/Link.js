...
const ActiveLink = ({children, className: activeClassName, ...props}) => {
	...
	const className = asPath === props.href || asPath === props.as ? `${childClassName} ${activeClass}`.trim() : childClassName
...
const CustomLink = ({href, children, isActive, activeClassName, ...rest}) => {
	...
	if(isInternalLink){
		return (
				<BaseLink href={href} className={clsx("cursor-pointer", activeClassName)} {...rest}>
					{children}
				</BaseLink>)
	}
