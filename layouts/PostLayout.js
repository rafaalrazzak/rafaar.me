...
export default function PostLayout({
	frontMatter, authorDetails, next, prev, availableLocales, children,
}){
	const {slug, fileName, created_time, title, tags, readingTime, image, author} = frontMatter
	...
	function SideBar(){
		...
		return (
				<div
						className={`${isTop ? 'xl:top-0 xl:flex xl:flex-col ' : 'xl:sticky xl:top-12 xl:flex xl:flex-col'} hidden`}
				>
					<div
							className={`${isTop ? 'mt-0' : 'mt-12'} pb-6 transition-all duration-700 xl:border-b xl:border-secondary-200 xl:dark:border-secondary-700`}
					>
						<dt className="sr-only">{t('common:authors')}</dt>
						<dd>
							{Array.isArray(author) ? (
									author.map((singleAuthor) => <Author key={singleAuthor.name} {...singleAuthor} />)) : (
									<Author {...author} />)}
						</dd>
					</div>
			...
			<SectionContainer>
				...
				<article>
					<div className="divide-transparent xl:divide-y">
							...
							<div className="flex justify-center space-y-1 text-center">
								<dl className="space-y-10">
									...
									<dd className="flex items-center justify-center divide-x-2 divide-secondary-500 text-sm leading-6 text-secondary-500 dark:divide-secondary-400 dark:text-secondary-400">
										<TimeAgo datetime={created_time} className="px-2" locale={locale}/>
										<time className="px-2" dateTime={created_time}>
						...
						>
							<div
									className="pt-6 pb-10 xl:hidden xl:border-b xl:border-secondary-200 xl:pt-11 xl:dark:border-secondary-700">
								<dt className="sr-only">{t('common:authors')}</dt>
								<dd>
							...
							<div className="divide-y divide-transparent xl:col-span-3 xl:row-span-2 xl:pb-0">
								{image && (
										<div className="flex relative w-full h-64 sm:h-96 rounded-xl overflow-hidden justify-center">
											<Image src={image} alt={title} className="object-cover bg-cover bg-center" fill/>
										</div>)}
