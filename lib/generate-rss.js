...
const generateRssItem = (post, locale, defaultLocale) => `
  ...
  post.slug
    ...
    <pubDate>${new Date(post.created_time).toLocaleDateString(locale)}</pubDate>
...
const generateRss = (posts, locale, defaultLocale, page = 'feed.xml') => `
  <rss version="2.0" xmlns:atom="http://www.w3.org/2005/Atom">
    <channel>
      ...
      <lastBuildDate>${new Date(posts[0].created_time).toLocaleDateString(locale)}</lastBuildDate>
