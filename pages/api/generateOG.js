...
const JakartaSansBold = fetch(new URL('../../public/fonts/PlusJakartaSans-Bold.ttf', import.meta.url))
		.then((res) => res.arrayBuffer())
	...
	return new ImageResponse((
			...
			>
				...
				>
					...
					>
						{description && (
								<div style={{fontSize: 32, opacity: 0}}>{description}</div>)}

						<div
								style={{
									fontSize: 70, fontWeight: 700, fontFamily: 'Inter'
								}}
						>
							{title}
						</div>
		...
		width: 1200, height: 630, fonts: [{
			...
			name: 'Inter', data: interBoldFont, style: 'normal', weight: 700
		}]
	})
}